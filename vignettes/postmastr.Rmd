...
5. unit
6. house number
7. fractional house number
8. house suffix
9. street directionals
10. street suffix
11. street name
12. reconstruct
...
2. unit
3. house number
4. fractional house number
5. house suffix
6. street directionals
7. street suffix
8. street name
9. reconstruct
...
There is no dictionary for this step - the first word of any address will be parsed out so long as it contains at least some numbers.
...
Once we have parsed data, we can rebuild our street address strings with `pm_rebuild()`:

```r
> sushi2_min <- pm_rebuild(sushi2_min, start = pm.house, end = pm.streetSuf)
> sushi2_min
# A tibble: 11 x 6
   pm.uid pm.rebuilt           pm.house pm.preDir pm.street   pm.streetSuf
    <int> <chr>                <chr>    <chr>     <chr>       <chr>       
 1      1 3407 Olive St        3407     NA        Olive       St          
 2      2 3221 S Grand Blvd    3221     S         Grand       Blvd        
 3      3 1 Maryland Plz       1        NA        Maryland    Plz         
 4      4 4949 W Pine Blvd     4949     W         Pine        Blvd        
 5      5 3674 Forest Park Ave 3674     NA        Forest Park Ave         
 6      6 1013 Washington Ave  1013     NA        Washington  Ave         
 7      7 3043 Olive St        3043     NA        Olive       St          
 8      8 308 N Euclid Ave     308      N         Euclid      Ave         
 9      9 910 Olive St         910      NA        Olive       St          
10     10 910 Olive St         910      NA        Olive       St          
11     11 4 N Euclid Ave       4        N         Euclid      Ave    
```

This function adds a variable named `pm.rebuilt` to the beginning of our `postmastr` object after `pm.uid`. The `start` and `end` arguments are useful for rebuilding only part of a parsed street address (such as to exclude city, state, and postal code data). For longer addresses, there are options to include commas before and after the city name.

With our addresses rebuilt, we can add the parsed data back into our original data set with `pm.replace()`:

```r
> pm_replace(sushi2_min, source = sushi2, keep_parsed = "no", keep_ids = FALSE)
# A tibble: 15 x 4
   name                            address                visit    pm.rebuilt          
   <chr>                           <chr>                  <chr>    <chr>               
